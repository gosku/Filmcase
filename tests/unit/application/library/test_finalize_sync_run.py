...
from src.domain.library.operations import PruneResult, UncoveredResult
...
_UNCOVER = "src.application.usecases.library.finalize_sync_run.library_operations.remove_images_no_longer_covered"
...
def _uncovered(*, removed: int = 0, found: int = 0, skipped_reason: str = "") -> UncoveredResult:
    return UncoveredResult(
        uncovered_found=found,
        removed=removed,
        total=0,
...
class TestFinalizeSyncRun:
    def test_prunes_and_completes_when_it_wins_the_election(self, _no_other_folders):
        ...
        with (
            ...
            patch(_UNCOVER, return_value=_uncovered()),
        ...
        run.record_removal_results.assert_called_once_with(
            missing_found=1, uncovered_found=0, removed=1, skipped_reason=""
        )

    def test_clears_up_after_a_path_change_alongside_the_prune(self, _no_other_folders):
        run = _run()
        with (
            patch(_BEGIN, return_value=True),
            patch(_GET_FOLDER),
            patch(_PRUNE, return_value=_pruned(removed=2)),
            patch(_UNCOVER, return_value=_uncovered(removed=3, found=3)) as uncover,
            patch(_COMPLETE),
        ):
            finalize_sync_run(run=run)

        uncover.assert_called_once()
        run.record_removal_results.assert_called_once_with(
            missing_found=2, uncovered_found=3, removed=5, skipped_reason=""
        )

    def test_does_not_clear_up_when_the_prune_was_deferred(self):
        run = _run()
        ...
        with (
            ...
            patch(_UNCOVER) as uncover,
            patch(_COMPLETE),
        ):
            finalize_sync_run(run=run)

        uncover.assert_not_called()
