...
class Command(BaseCommand):
    ...
    def handle(self, *args: object, **options: Any) -> None:
        ...
        if result.images_uncovered:
            self.stdout.write(
                f"  {result.images_uncovered} of those were no longer inside any library folder,"
                " after a folder's path changed. Their files are untouched."
            )
    ...
    def _report(self, *, warning: sync_library_usecase.PruneWarning) -> None:
        ...
        uncovered_note = (
            f" ({warning.uncovered_found} of them no longer inside the folder after a path change)"
            if warning.uncovered_found
            else ""
        )
        self.stdout.write(
            self.style.WARNING(
                f"  Skipped removing {warning.missing_found} of {warning.total} image(s)"
                f" in {warning.folder_path}{uncovered_note} (safety guard)."
