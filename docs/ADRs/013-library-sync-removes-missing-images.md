...
The guard nonetheless **ships disabled**, with both thresholds defaulted out of reach. Overriding a tripped guard is possible only from the command line, and the Library page has no equivalent, so a guard firing on a folder repointed from that page would leave the gallery stale with nothing on the page able to resolve it. Between a stale gallery with no visible remedy and a removal that deletes no file from disk, the stale gallery is the worse outcome for a single-user local app. This is a default, not a design change: the mechanism is built and tested, and enabling it is two env values. Consequence 6 below is the work that should precede turning it on.
